...
<p class="mb-4 text-sm font-light text-[#EFF0F6]">
  ...
  <DefaultPage placeholder="Search For a Dispersement">
    ...
    <Table :pending="pagination.pending.value" :headers="{
      head: ['Customer Name', 'Schedules', 'Missed payment', 'Account Number', 'Branch', 'Amount', 'Status', 'actions'],
      row: ['customerName', 'depositDate', 'missedPayment', 'accountNumber', 'insurance', 'quotationAmount', 'quotationStatus'],
    }" :rowCom="Status_row" :rows="useQuations.quotations || []" :Fallback="TableRowSkeleton">
    </Table>
  </DefaultPage>
</template>
